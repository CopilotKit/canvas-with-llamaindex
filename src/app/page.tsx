...
import { mergeAgentStateSnapshots } from "@/lib/canvas/merge";
...
export default function CopilotKitPage() {
  ...
  // (refactored) Merge handled by mergeAgentStateSnapshots in lib/canvas/merge
  ...
  useEffect(() => {
    ...
    if (!incoming) return;
    cachedStateRef.current = mergeAgentStateSnapshots(cached, incoming);
  }, [state]);
  ...
  useCopilotAction({
    ...
    handler: ({ type, name }: { type: string; name?: string }) => {
      ...
      if (normalized && recent && recent.type === t && (recent.name ?? "") === normalized && now - recent.ts < 2000) {
        return recent.id;
      }
